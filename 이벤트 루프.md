...
※ 주의 : 아직 실행하지 않는다! Call Stack에서 처리해야 할 것들이 남아있다.

3. Call Stack에 있는 이벤트(위 예시에선 for문과 console.log("foo"))가 전부 처리되면,
그제야 microTask Queue 내부의 함수가 실행이 된다.

4. 4칸으로 채워진 messageQueue가 json 변수에 옮겨지고 처리된다.
