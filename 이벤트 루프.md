...
```txt
console.log라던지, 우리가 만든 function 등 여러 메소드, 함수 등을 넣는 스택.
...
3-1. wepAPIs는 setTimeout에서 설정한 시간만큼 countdown을 한다.
...
그런데 Call Stack에 코드가 남아있으면 rendering을 못한다
...
주의! js script가 Call Stack에 깔려있기 때문에 웹에서 마우스 클릭했을 때와 완전히 다른 결과가 나온다.
...
```txt
1번 코드는 if문에서 작업이 stack단에서 처리되고, else문에선 작업이 microtask단에서 처리된다.

if와 else에서 처리하는 단이 다르다.

그러면 'getData()를 호출하는 코드들의 상태에 따라' 실행 순서가 꼬이거나, 심지어는 Data 로드가 제대로 안 될 수도 있다.
getData()를 호출하는 쪽의 코드가 Call Stack 단에서만 처리될지, task 단에서만 처리될지, microTask단에서만 처리될지,
그런 건 코드마다 다 제각각일 테니 말이다.
...
이를 통해 실행 순서를 일정하게 유지할 수 있도록 보장한다.
...
// 위와 같은 for문처럼 간단한 요청 몇 개가 아니라 여기저기서 수많은 요청이 들어왔다고 생각해보자.
//  queueMicrotask(() => console.log(json))는 Promise처럼 microtask단으로 처리하기 위해 쓴 코드다.
// 실제로는 console.log(json) 말고 fetch라던지, Promise를 사용하는 코드라고 생각하고 보자.
```
...
※ 주의 : 아직 실행하지 않는다! 뭘 할지 '지시 사항'만 등록한 것이다! Call Stack에서 처리해야 할 것들이 남아있다.
