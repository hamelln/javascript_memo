...
```txt
console.log, 우리가 만든 function 등을 넣는 스택.
...
4. 카운트다운이 끝나면 setTimeout() 내부 함수(task)는 
web APIs => task Queue로 전달.

※카운트다운이 끝나면 바로 실행하는 게 아니다.
task queue로 전달할 뿐이다.

5. Call Stack이 비워지면 Task queue에 있는 작업이 Call stack으로 이동.

6. Call Stack으로 옮겨졌으니 실행하고 비운다.
...
> onClick()같은 이벤트도 taskQueue단에서 처리한다.

```txt
몇 개 console.log 찍는 정도는 괜찮다. 

그런데 시간이 오래 걸리는 작업이면?
혹은 작업량이 너무 많으면?

콜스택 비우는 데에 시간이 상당히 걸린다.
브라우저는 보통 초당 60번(60fps) 화면을 repaint한다.

그런데 Call Stack에 코드가 남아있으면? 
rendering할 시간이 어딨어. 일해야지.

그러면? 텍스트 선택, 마우스 클릭같은 이벤트도 다 막힌다.
노예는 일하느라 바쁘다.

만약 모든 이벤트를 동기적으로 처리했다간?
사용자는 떠나고 회사는 망하고 개발자는 굶주림에 허덕인다.

그러니 적당히 비동기를 잘 이해하고 써먹자.

※ 비동기라고 표현하지만 한 번에 다중작업을 하는 건 절대 아니다. 
js가 결국엔 단일 스레드라는 걸 잊지 말자.

얘네는 사람처럼 물 마시면서 유튜브 보고, 숨쉬고, 
발 꼼지락거리고...그런 식으로 동시에 여러 작업 못한다. 

다만 동시에 여러 작업을 하는 것처럼 보이게 할 뿐이다.
집중해야할 작업이 있어도 도중에 숨은 쉬게 처리하고,
눈도 깜빡이게 처리하고, 해야할 것들 순서를 분류하고 지시하는 것.
